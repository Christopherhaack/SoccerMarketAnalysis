...
class TransferSpider(scrapy.Spider):
    ...
    with open(filename, 'w') as f:
        f.write('Name, Country 1, Country 2, Age, Position, Previous Club, Next Club, Transfer Fee\n')
                        ...
                        for row in section.css('tbody tr'):
                            if row.css('td::text').extract()[0] != ' No arrivals' and row.css('td::text').extract()[0] != ' No departures':
                                ...
                                if typeTrans == 'Arrivals':
                                    f.write(name + ', ' + country1 + ', ' + country2 + ', ' + age + ', ' + position + ', ' + difClub +', ' + curClub + ', ' + season 
                                        + ', ' + year + ', ' + fee + '\n')
                                else:
                                    f.write(name + ', ' + country1 + ', ' + country2 + ', ' + age + ', ' + position + ', ' + curClub +', ' + difClub + ', ' + season
                                        + ', ' + year + ', ' + fee + '\n')
